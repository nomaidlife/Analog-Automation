...
type Candidate = { brand_name: string; us_approval_year?: number; ta?: string; route?: string; score: number; product_id?: number; };
...
export default function Page() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-4">
      ...
      <div className="text-xs text-gray-500">API: <code>{BACKEND_URL}</code></div>

      <div className="flex gap-2">
        <input
          value={query} onChange={(e) => setQuery(e.target.value)}
          placeholder="Type PAH, Opsynvi, gMG, etc."
      ...
      {err && <div className="text-sm text-red-600 border border-red-200 rounded-2xl p-3">{err}</div>}
